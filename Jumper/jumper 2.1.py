...
class JumperGame(GameBase):
    ...
    def update(self):
        ...
        if self.state == "MENU":
            ...
            if (keys[pygame.K_DOWN] or keys[pygame.K_s]) and not self.is_jumping:
                ...
                self.player_rect.width, self.player_rect.height = 59, 30
                self.player_rect.y = ALTURA_SUELO - 30
            else:
                self.is_ducking = False
                self.player_rect.width, self.player_rect.height = self.dino_w, self.dino_h
    ...
    def spawn_obstacle(self):
        if self.score > 100 and random.random() < 0.35:
            h_y = random.choice([ALTURA_SUELO - 90, ALTURA_SUELO - 60, ALTURA_SUELO - 30])
    ...
    def draw_dino(self, screen, color):
        ...
        eye_x = self.player_rect.x + (45 if self.is_ducking else 30)
        eye_y = self.player_rect.y + (10 if self.is_ducking else 8)
        pygame.draw.rect(screen, bg_color, (eye_x, eye_y, 4, 4))
        
        if not self.is_jumping and self.state == "PLAYING":
            pata_offset = 5 if self.step_index < 7 else (self.player_rect.width - 15)
            pygame.draw.rect(screen, bg_color, (self.player_rect.x + pata_offset, self.player_rect.bottom - 8, 10, 8))
    ...
    def draw(self, screen):
        ...
        if self.state == "MENU":
            ...
            self.draw_text(screen, "JUMPER", self.font_title, (200, 200, 200), BASE_WIDTH//2 + 5, BASE_HEIGHT//3 + 5)
            self.draw_text(screen, "JUMPER", self.font_title, DIA_TEXTO, BASE_WIDTH//2, BASE_HEIGHT//3)
            pygame.draw.rect(screen, DIA_TEXTO, (BASE_WIDTH//2 - 150, BASE_HEIGHT//3 + 60, 300, 5))
            ...
            for obs in self.obstacles:
                ...
                if obs['type'] == 'pajaro':
                    wing_y = obs['rect'].y - 10 if self.step_index < 7 else obs['rect'].y + 20
                    pygame.draw.rect(screen, txt_color, (obs['rect'].x + 10, wing_y, 15, 10))
